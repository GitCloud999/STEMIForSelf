...
//                    startAndEndIndexOfPoints.addPEnd(pRangeEndRR);
                    //  Extract ECG data in the dynamic range
//                  New changes added on 28 Apr 2025
//                    ArrayList<double[]> list2 = filters.pointDetectionButterFilter(5, pWaveBand, "bandpass");
                    ArrayList<double[]> list2 = filters.pointDetectionButterFilter(5, pWaveBand, "bandpass");
...
//                    ldh.viewData(filtSeg);
//                    System.out.println(pIdxLocalRR);
                    pIdxRR = pRangeStartRR + pIdxLocalRR;   //  Convert to global index
                    double[] segRR = new double[pWindowDataRR.length];
                    double[] slopes = new double[pWindowDataRR.length-1];
                    segRR = pWindowDataRR;
                    double dt = 1/fs;
...
//                    duration.PR.add( Math.max(0, (currentStart - qIdx) / fs * 1000 ) );
...
//        ldh.viewData(ecg);
        ...
        int searchStart = sIdx + (int) Math.round(0.005 * fs);
...
//     updated on 9 May 2025
    public int detectSPeak(double[] ecg, int rIdx, double fs, double baseline,
        ...
        int sIdx;
        int searchStart = rIdx + (int) Math.round(0.02 * fs);
        int searchEnd = rIdx + (int) Math.round(0.04 * fs);
